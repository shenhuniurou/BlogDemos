...
public class BottomBar extends RelativeLayout implements View.OnClickListener, View.OnLongClickListener {
    ...
    public void setOnMenuTabClickListener(@Nullable OnMenuTabClickListener listener) {
        ...
        if (mMenuListener != null && mItems != null && mItems.length > 0 && mItems instanceof BottomBarTab[]) {
            listener.onMenuTabSelected(((BottomBarTab) mItems[mCurrentTabPosition]).id);
        }
    }
    ...
    public BottomBarBadge makeBadgeForTabAt(int tabPosition, int backgroundColor, int initialCount) {
        ...
        if (mIsComingFromRestoredState && mBadgeStateMap != null && mBadgeStateMap.containsKey(tabPosition)) {
     ...
     * @return
     */
    private boolean checkApkInstalled(Context context, String packageName) {
    ...
    private void updateItems(final BottomBarItemBase[] bottomBarItems) {
        ...
        for (BottomBarItemBase bottomBarItemBase : bottomBarItems) {
            ...
            if (mIsShiftingMode && !mIsTabletMode) {
                ...
                layoutResource = mIsTabletMode ? R.layout.bb_bottom_bar_item_fixed_tablet : R.layout.bb_bottom_bar_item_fixed;
            }
            ...
            /*if (checkApkInstalled(mContext, "com.ehome.skin.xmas")) {
                icon.setBackgroundDrawable(bottomBarItemBase.getIcon(mContext));
            } else {
                icon.setBackgroundResource(bottomBarItemBase.getBgResource());
            }*/

            if (getUninstallApkInfo(mContext, Environment.getExternalStorageDirectory().toString() + "/dynamicload/download/skin-plugin.apk") == null) {
                //包名为null说明指定路径下没有需更新的apk资源包，就使用原有的图片资源
                icon.setBackgroundResource(bottomBarItemBase.getBgResource());
            } else {
                //否则使用资源apk中的图标资源
                icon.setBackgroundDrawable(bottomBarItemBase.getIcon(mContext));
    ...
    private void selectTab(View tab, boolean animate) {
        ...
        if (animate) {
            ...
            icon.setPadding(icon.getPaddingLeft(), mSixDp, icon.getPaddingRight(), icon.getPaddingBottom());
